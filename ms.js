 ...
 *
 * Fragmentary Customized be Valera Leontyev
 */
...
!function($) {
	...
	$.fn.multiselectable = function(options) {
		...
		function click(e) {
			if ( $(this.parentNode).is('.ui-draggable-dragging') ) {
				return;
			}

			var item = $(this.parentNode),	parent = item.parent();
		...
		return this.each(function() {
			...
			if (!list.data('multiselectable')) {
				list.data('multiselectable', true)
					.delegate(options.handle, 'mousedown', mouseDown)
					.delegate(options.handle, 'click', click)
					// .disableSelection();
			}
		})
	};
